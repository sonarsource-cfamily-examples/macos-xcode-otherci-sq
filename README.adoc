...
.. `-Dsonar.cfamily.compile-commands=bw-output/compile_commands.json` (with `bw-output` being the same directory as the one passed to Build Wrapper as `--out-dir`) if you are using SonarQube version 10.6 or later
.. `-Dsonar.cfamily.build-wrapper-output=bw-output` if you are SonarQube version 10.5 or earlier
